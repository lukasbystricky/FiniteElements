...
/***************************************************************************************************
 * Time dependent Stokes flow solver
 * 
 * Author: Lukas Bystricky, lb13f@my.fsu.edu
 * 
 * Solves Stokes equations on the unit box with Dirichlet boundary conditions on the velocity. 
...
class StokesSolver
{
	...
	private:
		...
		struct InputFile
        {
            ...
            double       mu, t0, tf, dt, r_in, r_out, domain_length, domain_height;
		...
		Vector<double>         system_rhs_slit_flow;
 ...
 **************************************************************************************************/
template<int dim>
void StokesSolver<dim>::read_inputs()
{
	/***********************************************************************************************
	...
	{
		prm.declare_entry("mu", "1", Patterns::Double(1e-7, 10000), 
...
{  		
    ...
    system_rhs_slit_flow.reinit(dof_handler.n_dofs());     
}

template<int dim>
void StokesSolver<dim>::assemble_matrix()
{
	if (input.verbose)
	{
    ...
    for (; cell!=endc; ++cell)
    {
			...
			{		
				for (int j = 0; j < dofs_per_cell; j++)
				{
					...
					else
					{
						cell_matrix(i,j) += 
								(phi_u[i]*phi_u[j]
										+ input.mu*input.dt*double_contract(grad_phi_u[i],grad_phi_u[j])
		...
		for (int i = 0; i < dofs_per_cell; i++)
		{
			system_rhs_slit_flow[local_dof_indices[i]] += cell_rhs(i);
...
template<int dim>
void StokesSolver<dim>::solve_time_step()
{
	if (input.verbose)
	{
...
template<int dim>
void StokesSolver<dim>::output_results(int cycle)
{	
...
template<int dim>
void StokesSolver<dim>::calculate_error(int cycle, double t)
{
...
void StokesSolver<dim>::run_time_loop()
{
	...
	{	
		...
		system_rhs += system_rhs_slit_flow;
		system_rhs *= input.dt;	
		system_rhs.add(tmp);
		
		std::vector<bool> boundary_dofs(dof_handler.n_dofs(), false);
		DoFTools::extract_boundary_dofs(dof_handler, fe.component_mask(pressure), boundary_dofs);
...
template<int dim>
void StokesSolver<dim>::print_errors()
{
