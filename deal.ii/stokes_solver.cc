...
/***************************************************************************************************
 * Steady state Stokes flow solver
 * 
 * Author: Lukas Bystricky, lb13f@my.fsu.edu
 * 
 * Solves steady state Stokes equations on the unit box with Dirichlet boundary conditions on the 
 * velocity. 
 * 
...
class StokesSolver
{
	...
	private:
		...
		struct InputFile
        {
            ...
            double       mu, r_in, r_out, domain_length, domain_height;
    ...
    for (; cell!=endc; ++cell)
    {
        ...
        for (int q = 0; q < n_q_points; q++)
        {
			...
			for (int i = 0; i < dofs_per_cell; i++)
			{
				...
				cell_rhs[i] += fe_values.shape_value(i,q)*rhs_values[q](equation_i)*fe_values.JxW(q);  
			}
		}
...
template<int dim>
void StokesSolver<dim>::calculate_error(int cycle)
{
